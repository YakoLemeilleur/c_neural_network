...
void network_train(NeuralNetwork* net, Matrix* input, Matrix* output) {
	...
	//printf("Hidden outputs: %dx%d\n",hidden_outputs->rows,hidden_outputs->cols);
	//printf("output_weights: %dx%d\n",net->output_weights->rows,net->output_weights->cols);
...
void network_train_batch_xors(NeuralNetwork* net, int** inputs, int batch_size) {
	for (int i = 0; i < batch_size; i++) {
		int* current_input = inputs[i];
		Matrix* inputs_data = matrix_create(2, 1);
		inputs_data->entries[0][0] = current_input[0];
		inputs_data->entries[1][0] = current_input[1];
		Matrix* output = matrix_create(1, 1);
		output->entries[0][0] = current_input[0]^current_input[1];
		network_train(net, inputs_data, output);
		matrix_free(output);
		matrix_free(inputs_data);
	}
}
...
Matrix* network_predict_xor(NeuralNetwork* net, int* inputs) {
	Matrix* inputs_data = matrix_create(2,1);
	inputs_data->entries[0][0] = inputs[0];
	inputs_data->entries[1][0] = inputs[1];
	Matrix* res = network_predict(net, inputs_data);
	matrix_free(inputs_data);
	return res;
}
...
double network_predict_xors(NeuralNetwork* net, int** inputs, int n) {
	int n_correct = 0;
	for (int i = 0; i < n; i++){
		Matrix* prediction = network_predict_xor(net, inputs[i]);
		if (matrix_closest(prediction) == (inputs[i][0]^inputs[i][1])) {
			n_correct++;
		}
		matrix_free(prediction);
	}
